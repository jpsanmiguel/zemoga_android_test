...
class ConnectionLiveData(context: Context) : LiveData<Boolean>() {
    ...
    fun hasInternetConnection() = this.value == true
    ...
    object DoesNetworkHaveInternet {
        private const val GOOGLE_PUBLIC_DNS = "8.8.8.8"
        private const val PORT_ID = 53
        private const val TIME_OUT_MILLIS = 1500

        // Make sure to execute this on a background thread.
        fun execute(socketFactory: SocketFactory): Boolean {
            return try{
                val socket = socketFactory.createSocket() ?: throw IOException("Socket is null.")
                socket.connect(InetSocketAddress(GOOGLE_PUBLIC_DNS, PORT_ID), TIME_OUT_MILLIS)
                socket.close()
                true
            }catch (e: IOException){
                false
